...
namespace DTSchema.Compiler.Backends
{
    ...
    */
    public sealed class TypeScript : Backend
    {
        ...
        {
            _ = P << $"import * as {Required} from \"./{Required}.ts\";";
            _ = P << $"export * from \"./{Required}.ts\"";
            _ = P << "";
        ...
        string ShowSerdeNoFunc(Ty _, string expr) => expr;
        ...
        string ShowSerdeHasFunc(Ty t, string expr, bool nullable)
        {
            switch (t)
            {
                case TyFn fn:
                    if (nullable)
                        return string.Format("((_x) => (_x === null || _x === undefined) ? null : {0}.signal(_x))({1})", Required, expr);
                    else
                        return string.Format("{0}.signal({1})", Required, expr);
                case TyList list:
                    if (nullable)
